...
ap_path = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
...
def apply_constraints(game_options, constraints, option_defs):
    ...
    mutual_exclusions = [
        {"option": c.get("option"), "values": c["mutually_exclusive"]} for c in constraints if "mutually_exclusive" in c
    ]
    ...
    for excl in mutual_exclusions:
        _apply_single_constraint(
            game_options,
            {"option": excl["option"], "mutually_exclusive": excl["values"]},
            mutual_exclusions,
            option_defs,
        )
...
def generate_random_yaml(world_name, meta):
    ...
    for group, options in option_groups.items():
        ...
        for option_name, option_value in options.items():
            ...
            game_options[option_name] = sanitize(get_random_value(option_name, option_value))
...
def get_random_value(name, option):
    ...
    if issubclass(option, OptionCounter):
        ...
        if not option.valid_keys:
            return option.default
        selected_keys = random.sample(list(option.valid_keys), k=random.randint(0, len(option.valid_keys)))
    ...
    if issubclass(option, OptionSet):
        return random.sample(list(option.valid_keys), k=random.randint(0, len(option.valid_keys)))
    ...
    if issubclass(option, FreeText):
        special_symbols = "&<>\"'\\/@#$%^*()[]{}|;:,."
        whitespace = " \t\n"
        multibyte_utf8 = "ÀÁÂÃÄÅÆÇÈÉÊËΒΓΔбвг中文日本語한글🎮🎯🎲🔥💀𝕳𝖊𝖑𝖑𝖔"
...
def gen_wrapper(yaml_path, apworld_name, i, args, queue, tmp):
    ...
    try:
        with (
            ...
            redirect_stderr(out_buf),
            tempfile.TemporaryDirectory(prefix="apfuzz", dir=tmp) as output_path,
        ):
            try:
                ...
                if raised:
                    ...
                    if not is_option_error and isinstance(raised, PlayerFilesError):
                        is_option_error = all(exception_in_causes(e, OptionError) for e in raised.exceptions)
    ...
    try:
        ...
        if not IS_TTY:
            ...
            if step == 0 or (checks_done % step) == 0:
                print(
                    f"{checks_done} / {args.runs} done. {FAILURE} failures, {TIMEOUTS} timeouts, {OPTION_ERRORS} ignored."
                )
...
def find_hook(hook_path):
    ...
    if issubclass(obj, BaseHook):
        raise RuntimeError(f"the hook {hook_path} is not a subclass of `fuzz.BaseHook`)")
...
if __name__ == "__main__":
    ...
    def main(p, args, tmp):
        ...
        if args.sample_from:
            if args.game:
                raise Exception("--sample-from is incompatible with -g/--game")
            if args.meta:
                raise Exception("--sample-from is incompatible with -m/--meta")
        ...
        for apworld in apworld_names:
            if world_from_apworld_name(apworld) is None:
                raise Exception(f"Failed to resolve apworld from apworld name: {apworld}")
        ...
        i = 0
        valid_worlds = [world.__module__.split(".")[1] for world in AutoWorldRegister.world_types.values()]
        ...
        if args.sample_from:
            ...
            if not sample_yamls:
                raise Exception(f"--sample-from directory {args.sample_from!r} contains no YAML files")
        ...
        while i < args.runs:
            if len(yamls_per_run_bounds) == 1:
                ...
                yamls_this_run = random.randrange(yamls_per_run_bounds[0], yamls_per_run_bounds[1] + 1)

            if args.sample_from:
                actual_apworld = "sample"
                yamls_to_write = [
                    (f"sample-{i}-{nb}-{orig_name}", content)
                    for nb, (orig_name, content) in enumerate(random.sample(sample_yamls, yamls_this_run))
                ]
            else:
                ...
                yamls_to_write = [
                    ...
                    for nb, game in enumerate(g for g in games_this_run for _ in range(yamls_this_run))
                ]
