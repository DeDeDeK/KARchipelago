...
class DolphinInterface:
    ...
    def check_game_running(self) -> bool:
        ...
        A wrong or empty game id returns False."""
        return self.read_bytes(MemoryAddress.MEM1_START, 6) == self.kar_game_id

    def resolve_ap_data(self) -> int | None:
        """Read the APData struct pointer, or None if not yet allocated. Before the mod writes this,
        the memory can be garbage, so is rejected until the pointer exists."""
