...
# Item types that get a guaranteed copy, or their counted quantity without being progression.
_COUNTED_USEFUL_TYPES: frozenset[KARItemType] = frozenset(
    {
        KARItemType.CHECKBOX_FILLER,
        KARItemType.SPAWN_RATE,
        KARItemType.PATCH_CAP_INCREASE,
        KARItemType.CT_EVENT_UNLOCK,
    }
)
...
class _CapacityModel(NamedTuple):
    ...
    total_default: int
    total_excluded: int
    # In-pool checklist rewards needing a default box
    useful_rewards: int
...
class KARWorld(World):
    ...
    # one from the slot data instead.
    ...
    def _determine_starter_items(self) -> None:
        """
        One starter per gated category. Deliberately skipped as playable without: events, abilities, boxes,
    ...
    def _compute_logic_modes(self) -> set[GameMode]:
        """
        The modes whose region trees get built: every mode with a goal, plus the modes the AP checklist's
        boxes sit in.
        """
        modes = {
            mode
    ...
    def _category_holds_keys(self, cat: GatingCategory) -> bool:
        """
        Whether a category holds unlock items this seed.
        """
    ...
    def _goal_required_unlocks(self) -> set[str]:
        """
        The unlocks this seed's goal is gated on, whatever their category's gate says.
        """
    ...
    def _compute_goal_forced_unlocks(self) -> set[str]:
        """
        The goal's keys the pool must ship even though their category's gate is off.
        """
    ...
    def _validate_options(self) -> None:
        ...
        # Snap spawn rate ups to a multiple of 10
    ...
    def _item_classification(self, name: str, data: KARItemData) -> ItemClassification:
        options = self.options
        if options.city_trial_goal.value != CityTrialGoal.option_max_stats_in_one_run:
            if data.type == KARItemType.PATCH_CAP_INCREASE:
                return ItemClassification.useful
            return data.classification

        if name in MAX_STATS_GOAL_KEYS and options.city_trial_patches_gated and options.city_trial_items_gated:
            return data.classification & ~ItemClassification.deprioritized
        return data.classification
    ...
    def _build_item_pools(self) -> None:
        ...
        # A checklist reward category left out of `checklist_rewards` is unlocked by the mod instead
        for category, names in CHECKLIST_REWARD_CATEGORIES.items():
            if category not in self.options.checklist_rewards.value:
                excluded |= names

        # likewise for allowed items
        ...
        # Spawn Rate Up: excluded when the ceiling is at or below the min.
        ...
        # Backstop: an item whose source_modes miss every mode being played has no in-game effect here.
    ...
    def _apply_ut_passthrough(self) -> None:
        ...
        during a real generation, since only UT's MultiWorld has `re_gen_passthrough`."""
    ...
    def interpret_slot_data(slot_data: dict[str, Any]) -> dict[str, Any]:
        ...
        rather than the player's local YAML."""
        return slot_data
    ...
    def generate_early(self) -> None:
        ...
        self.ap_patch_count = self.options.ap_patches.value if GameMode.CITYTRIAL in self.logic_modes else 0
        group_sizes = ap_patch_group_sizes(self.ap_patch_count)
        self.ap_patch_group_count = len(group_sizes)
        self.ap_patch_locations = {}
        entries = list(AP_PATCH_LOCATION_TABLE.items())[: self.ap_patch_count]
        offset = 0
    ...
    def _compute_capacity(self) -> _CapacityModel:
        ...
        # Progression part-markers live in progression_pool, reward_pool is purely useful/filler.
        useful_rewards = sum(
    ...
    def _validate_allowed_items_filler(self) -> None:
        if not self.filler_pool:
            raise OptionError("Filler pool is empty!")

    def _validate_pool_fits_locations(self, cap: _CapacityModel) -> None:
        """Verify the guaranteed item pool fits the available locations"""
    ...
    def create_item(self, name: str) -> KARItem:
        if name in self.item_names or name in ITEM_TABLE:
            ...
            return KARItem.from_data(str(name), self.player, data, self._item_classification(name, data))
    ...
    def create_items(self) -> None:
        ...
        # One filler per excluded location, minus the filler rewards already destined for those boxes.
    ...
    def _random_trap(self) -> str | None:
        """Pick a random active trap, or None if no trap categories are active."""
        if not self.trap_pool:
            return None
        return self.random.choice(sorted(self.trap_pool))

    def _random_filler(self) -> str:
        """Random filler from the built filler_pool."""
    ...
    def _checklist_reward_placed_types(self) -> int:
        """Bitmask of the (mode, RewardType) pairs this seed placed as AP items."""
        mask = 0
    ...
    def fill_slot_data(self) -> Mapping[str, Any]:
        """
        Only options the client or mod actually consume are shipped.
        """
        slot_data = dict(
        ...
        # The count this seed actually created, which could be different from the raw option value
        slot_data["ap_patches"] = self.ap_patch_count

        # The reward types this seed placed
